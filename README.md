...
| **v0.7.1** *(current)* | Cross-domain validation (Metacoherence) — **shipped + tagged 2026-06-24** as the D₁ instrument-calibration tier (D₁ build + decoupling control + the program reframe + R1 calibration + the D₂ pre-registration); the cross-domain *validation* itself is **ongoing**, D₂ next. R1 (persistence) + R2 (cross-philosophy convergence) + R3 (intervention asymmetry) within-domain signatures + the M5 admissibility gate + the eight-cell outcome matrix, across three domains (D₁ synthetic HSMM; D₂ Pfam, CC0; D₃ FOMC, public domain). **v0.7.0 pre-registered**: the D₁ hidden-semi-Markov substrate (8 features encoding 4 properties + 3 distractors), its exact M5 partition, and R2 (median Spearman > 0.6) via the categorical 5×3 grid with marginal-relative coherence. Build sequenced D₁ → R1 → R3 → D₂ → D₃ → M5 capstone. **v0.7.0 D₁ built** (categorical marginal-relative K₁–K₅ + A₁/A₂/A₃ + R2/cross-tab machinery). Finding, recorded not tuned (and **record-corrected** to instrument-validity): on D₁'s A₁ column the proxies recover *complementary* properties (K₅ → A,B,C,D; K₁ → A,C; K₂/K₃/K₄ → A,D), but the induced-`w` Spearman blocks {K₁,K₅} (within +0.68) vs {K₂,K₃,K₄} (within 0.71–0.93) split **collinear with the byte-stream vs categorical-native encoder boundary** — so philosophy and *representation* are confounded and `R2 > 0.6` on D₁ is **not yet adjudicable** (not a falsification, not a vindication). Provisional (single seed, T=8000=16% of locked); only property **A** survives both representations and all five proxies. The representation-vs-philosophy **decoupling control** (modeling-on-byte-stream crossings K₃ᵇ/K₂ᵇ, A₁, full-T, 20 seeds) was **run** (2026-06-24): terminal verdict **INCONCLUSIVE** — necessary-not-sufficient. It *weakens* the representation-artifact reading (nothing leaned byte at full power) but does **not establish** philosophy: K₃ᵇ leans modeling confidently (20/20, median Δ +0.51) yet was pre-flagged as able to lean modeling for trivial flexibility reasons, while K₂ᵇ — the crossing that probes the actual factorized C-blindness — was unstable (17/20, one seed short of the locked 18/20). A pre-registered read-only I_C diagnostic **rejected** a property-dependent-representation explanation (the negative-Δ seeds are not high-I_C), so the K₂ᵇ near-miss is genuine borderline noise; option (e)(i) remains the recorded-but-unjudged next control. **Program reframe (2026-06-24, epistemics not results):** D₁'s role is recast as **estimator coverage-calibration** against known ground truth — the finding is **differential coverage with K₅ (parsing) most complete** — *asymmetric, not compositional*: K₅ is the widest aperture (A,B,C,D), the modeling trio gapped to {A,D}, compression to {A,C}, A universal; the ensemble-recovers-all is carried by K₅, not a composition (so on D₂/D₃ weight parsing as the widest net, the others as specialized/confirmatory — the ranking itself provisional: single seed, 16% T). The D₁ `R2 > 0.6` commitment is **superseded in status, not deleted**. **R2 is reclassified diagnostic** (coverage-capped — corroborating on convergence, non-falsifying on divergence, since cross-extractor `w` agreement can only certify shared-coverage structure; the 0.6/0.4 numbers stay as a convergence flag, the grid still runs). **R1 (persistence) + the v0.6.2 selective-compression functional win are elevated to primary** cross-domain evidence (functional, not weight-vector, convergence) — with R1's spec pre-registering *per-estimator* functional validity aggregated, **not** cross-estimator agreement on predictions, so the coverage ceiling isn't rebuilt under a functional label. The eight-cell matrix needs re-derivation under R2-as-diagnostic (capstone-pending). **v0.7.1 R1 (persistence) is built + run + closed** as **calibration** — *necessary-not-sufficient and mechanism-confounded*. A read-only re-read of the locked D₁ generator found its regime path is **emission-independent** (the Sec 5.4 drift→transition coupling is not instantiated), so D₁ "persistence" reduces to **regime-inference structure**; the apparatus (`cit/persistence_d1.py`) is validated against ground truth (distractors ~0; the relational B and coalitional C are per-feature-invisible by design), and the decoder-free log-likelihood-cost magnitude is **D-led, not A-dominated** — a pre-registered 20-seed diagnostic disambiguates it as *information-sensitive within a property* (Spearman(A-cost, I_A) = +0.835) **and** *concentration-biased across* (I_A ≈ I_D yet D costs ~6.7×). A partial proxy d-matrix (cheap cells; K₁ fails −0.386, K₂ passes +0.986) makes the confound concrete — "passing R1 on D₁" means weighting the sharp feature D, which coincides with the persistence-designated feature for the *concentration* reason, not persistence-relevance. The concentration confound is parked for D₂/D₃ behind a **pre-registered emission-concentration-homogeneity check** (the full R1 grid over K₃/K₄/K₅ and A₂ is deferred — predictable, confounded, slow). Suite at this work: **272 fast + 100 slow + 18 very_slow = 390** (1 xfail); v0.7 adds the categorical D₁ + decoupling-control + R1-persistence tiers (`test_crossing_proxies.py` 6+1, `test_decoupling_control.py` 20+1, `test_r1_persistence.py` 11) |
