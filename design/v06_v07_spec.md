...
Benjamin's rulings). Steps 4-6 then RULED c3/c1, PILOT-PASSED R1-EDGE on 3 families (incl. the clean-tree PF00348), and FALSIFIED K_comp's beyond-raw-MI persistence as a marginal-bias artifact (K_comp ~= raw MI +0.99) -> the multi-proxy construct is within-domain MIp + shared bias; the decisive test is CROSS-DISJOINT-DOMAIN TRANSFER. NEXT relational component = Benjamin's call (cross-domain transfer / D3). The D2 grid / family-lock / M5 runs stay HELD. Then D3 (FOMC); v0.7.2 R3; the M5 +
