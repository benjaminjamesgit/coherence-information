...
def main(mode):
    T = 6000 if mode == "dev" else 20000
    seeds = list(range(7000, 7003)) if mode == "dev" else list(range(7000, 7006))
    ...
    # ---------- induce w + base on the ensemble (real + base-matched null), both encoders ----------
    W1, W2, B1, B2, Wsh1, Wsh2 = {}, {}, {}, {}, {}, {}
    for s in seeds:
        ...
        B1[s], B2[s] = base_measure(O1, A1), base_measure(O2, A2)
        Wsh1[s], Wsh2[s] = induce_w(Os1, A1), induce_w(Os2, A2)
        print(f"  induced w+base seed {s}", flush=True)
    ...
    # provisional fork READ (NOT the official run -- smoke only)
    bmn_kills_w = (n_bmn_degen == len(w_bmn))      # base-matched null DEGENERATES w (no beyond-base structure left)
    w_beyond = (wr.mean() > wn.mean() + 0.2) and bmn_kills_w
    print(f"\n  provisional read (smoke, NOT official): w-transfer real {wr.mean():+.3f} >> across-latent null "
          f"{wn.mean():+.3f}; the base-matched null DEGENERATES w ({n_bmn_degen}/{len(w_bmn)} latents, all-0.5) -> "
          f"w's transmissible content IS the beyond-base RECUR structure; escape-leaning={w_beyond}", flush=True)
    print("  NOTE: provisional only; the FORK is the OFFICIAL run. The de Bruijn period is a long-range recursive", flush=True)
    print("  regularity a REAL compressor sees but same-time-pairwise-MI base does NOT -- whether that counts as", flush=True)
    print("  w-beyond-base coherence is Benjamin's adjudication. HOLD (he reviews gate-(i) base-matched null first).", flush=True)

    out = {"mode": mode, "T": T, "gates": gates,
           "gate_i": {"ck_real": ck_real, "ck_null": ck_null, "gap_enc1": gap1, "gap_enc2": gap2, "base_matched": base_ok},
           "w_by_role_enc1": wg, "base_by_role_enc1": bg,
           "transfers": {"w_real": float(wr.mean()), "w_acrosslatent_null": float(wn.mean()),
                         ...
                         "base_real": float(br.mean()), "base_acrosslatent_null": float(bn.mean())},
           "w_beyond_base_smoke": bool(w_beyond), "std_w_min": std_w}
